(ns evelyn.models.tick
  ...
  (:require [evelyn.entities :as entities]
            [evelyn.tick-processor.google :as google]
            [evelyn.util :as util]
...
(let [null-writer (proxy [java.io.Writer] []
                    ...
                    (flush [] nil)
                    (close [] nil))
      merge-stmt-prefix (str "MERGE INTO \"ticks\" (\"symbol\", \"time\", "
                             "\"close\", \"high\", \"low\", \"open\", "
                             "\"volume\") KEY (\"symbol\", \"time\") VALUES ")
      merge-stmt-suffix "(?,?,?,?,?,?,?)"]

  (defn add [vals]
    (try
      (binding [*out* null-writer]
        (let [merge-stmt (apply
                           str
                           (concat [merge-stmt-prefix]
                                   (interpose "," (repeat (count vals) merge-stmt-suffix))))
              merge-vals (apply
                           concat
                           (map #(identity
                                   [(name (:symbol %)) (.getMillis (:time %))
                                    (:close %) (:high %) (:low %) (:open %)
                                    (:volume %)])
                                vals))]
...
(let [eastern (DateTimeZone/forID "US/Eastern")]
  (defn update? [symbol]
    ...
    (let [last-weekday (util/preceding-weekday (DateMidnight. eastern))
...
(let [processors [(google/new)]]
  ...
  (defn backload [symbol]
    ...
    (when (update? symbol)
      (let [ticks (load-files processors symbol)]
